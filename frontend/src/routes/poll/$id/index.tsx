...
function TakePoll() {
  ...
  useEffect(() => {
    apiClient.get(`/polls/${id}`).then(res => {
      ...
      setPoll(data)
      if (data.expiresAt && new Date(data.expiresAt) < new Date()) setExpired(true)
    }).catch(() => alert('Poll not found')).finally(() => setLoading(false))
  }, [id])
  ...
  const submitResponse = async () => {
    ...
    } catch { alert('Failed to submit response') } finally { setSubmitting(false) }
  }

  const mandatory = poll?.questions?.filter((q: any) => q.isMandatory !== false) ?? []
  ...
  if (expired) return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
      ...
      <p className="text-muted-foreground text-sm">This poll is no longer accepting responses.</p>
    </div>
  )
    ...
    <div className="max-w-2xl mx-auto w-full px-4 py-10">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        <GlassCard className="p-6 mb-6">
          <h1 className="text-2xl font-bold tracking-tight mb-1">{poll.title}</h1>
          {poll.description && <p className="text-muted-foreground text-sm mb-4">{poll.description}</p>}
          {poll.expiresAt && (
            <p className="text-xs text-muted-foreground flex items-center gap-1 mb-3">
              <Clock className="w-3 h-3" /> Expires {new Date(poll.expiresAt).toLocaleString()}
            </p>
          )}
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-xs text-muted-foreground shrink-0">{answeredCount}/{totalQ}</span>
          </div>
        </GlassCard>
      </motion.div>
      ...
      <div className="space-y-4">
        {poll.questions?.map((q: any, i: number) => (
          <motion.div key={q.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}>
            <GlassCard className="p-6">
              <div className="flex items-start gap-3 mb-4">
                <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 mt-0.5 transition-colors ${
                  ...
                  {answers[q.id] ? <CheckCircle className="w-3.5 h-3.5" /> : i + 1}
                </span>
                <div>
                  <h3 className="font-medium leading-snug">{q.text}</h3>
                  {q.isMandatory === false && <span className="text-[10px] text-muted-foreground">Optional</span>}
                </div>
              </div>
                ...
                {q.options?.map((opt: any) => {
                  ...
                  return (
                    <label key={opt.id} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${
                      selected ? 'border-primary bg-primary/8 text-foreground' : 'border-border hover:border-primary/40 hover:bg-muted/50'
                    }`}>
                      ...
                      <input type="radio" name={q.id} value={opt.id} checked={selected}
                        onChange={() => setAnswers({ ...answers, [q.id]: opt.id })} className="sr-only" />
      ...
      <div className="mt-8 flex justify-end">
        <ShimmerButton onClick={submitResponse} disabled={!allMandatoryAnswered || submitting} className="h-11 px-8">
